...
function getProductMarkup(product) {
    ...
    product.forEach(item => {
        ...
        newProduct += `<div>${item.name}</div>`;
        newProduct += `<img src="${item.imageUrl}" alt="">`;
        newProduct += `<div>${item.price}</div>`;
        newProduct += `<a href="https://example.com/producs/${item.id}">Подробнее</a>`;
        newProduct += '</div>';
