...
buttonz.forEach(btn => btn.addEventListener('click', handleClick));
...
function showMoreText(card) {
    ...
    let addText = 'Прекрасный товар, покупайте!';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterEnd', `<div class="desc">${addText}</div>`);
